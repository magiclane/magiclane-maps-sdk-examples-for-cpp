...
int main(int argc, char** argv)
{
...

    {
        ...
        vehicle1.setEndTime(1860);   //7:00:00 PM in minutes
        ...
        //get all routes from database that contains string "optimization1"
        gem::vrp::RouteList allRoutes;
        res = serv.getRoutes(&listener, allRoutes, "optimization1");
        ...
        if (listener.IsFinished() && listener.GetError() == gem::KNoError && res == gem::KNoError)
            std::cout << allRoutes.size() << " routes returned successfully" << std::endl;
        else
            std::cout << "No route returned" << std::endl;
    }
