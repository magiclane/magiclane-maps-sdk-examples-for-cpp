...
- Add an optimization with orders visited in a custom order (not in fixed) and display the solution on the map.

Create an optimization where some orders must be visited in a specific not-fixed order.
...
2. Create a `vrp::ConfigurationParameters` and set the sequences to it.
