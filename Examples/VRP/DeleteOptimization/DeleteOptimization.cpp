...
int main( int argc, char** argv )
{
...

    {
        ...
        res = serv.deleteOptimization(&listener, { optimization.getId() });
        WAIT_UNTIL(std::bind(&ProgressListener::IsFinished, &listener), 5000);

        if (listener.IsFinished() && listener.GetError() == gem::KNoError && res == gem::KNoError)
            std::cout << "Optimization deleted successfully" << std::endl;
        else
            std::cout << "Optimization couldn't be deleted" << std::endl;
    }
