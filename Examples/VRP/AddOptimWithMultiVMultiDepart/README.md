...
- Add an optimization with multiple vehices which start their routes from different departures and display the solution on the map.
...
1. Create a `vrp::OrderList` and add the orders to it. Each order needs to have a customer set; you can either add a new customer and then set it to the order, or you can use a previously created customer (see [Get Customer](../GetCustomer)).
...
3. Create a `vrp::Optimization` and set the objects created at 1.) and 2.) to it. Also set other parameters such as departures.
...
6. After adding the optimization, monitor the request until it reaches a finished status. Once completed, retrieve the optimization results by calling the `getSolution()` method, which returns a `vrp::RouteList` containing the generated routes.
