...
int main( int argc, char** argv )
{
...

    {
        ...
        vehicle1.setStartTime(300); //5:00 AM in minutes
        vehicle1.setEndTime(1740);  //5:00 AM next day, in minutes
        ...
        vehicle2.setMaxWeight(3500);
        vehicle2.setMaxCube(10);
        vehicle2.setStartTime(300); //5:00 AM in minutes
        vehicle2.setEndTime(1800);  //6:00 AM next day, in minutes
        ...
        optimization.setDepartures({departure});   //both vehicles will start their route at the location of the departure and will return at it at the end of the route (by default the route is round)
        optimization.setVehiclesConstraints(vehConstraintsList);

        //display orders on map
        MapViewListenerImpl mapListener;
        auto oglContext = session.produceOpenGLContext(Environment::WindowFrameworks::Available, "AddOptimizationWithMultipleVehiclesSingleDeparture");
