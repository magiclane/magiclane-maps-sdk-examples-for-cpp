...
- Perform a stateless optimization using a single JSON input with fixed orders sequences set (the orders from a sequence will be visited in the fixed specified order).  
...
1. Create a JSON object containing all the optimization data:
   ...
   - **Order sequence options** (fixed sequences that must be visited in the specified order)
