...
- Perform a stateless optimization using a single JSON input with orders in same route sequence set (the orders from a sequence will be visited in the same route).  
...
1. Create a JSON object containing all the optimization data:
   ...
   - **Order sequence options** (same route sequences that must be visited in the same route)
