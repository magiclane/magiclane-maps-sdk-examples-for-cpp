...
int main( int argc, char** argv )
{
...

    {
        ...
        orderToAdd1.setCoordinates(gem::Coordinates(48.324664, -0.497207)); //the order will have the name, coordinates, address and phone number of the customer
        ...
        gem::FloatListList distancesToTheOtherOrders{
            gem::FloatList{41.059,635.625,320.045,321.041,825.147,628.418,523.937,147.393,423.374,630.276,190.460,328.325,180.552,578.046,256.942,148.753},
            gem::FloatList{296.675,553.544,465.368,201.964,606.572,635.096,157.627,284.942,261.019,360.122,175.278,57.356,445.646,495.965,346.734,451.026,254.871} };
        gem::IntListList timesToTheOtherOrders{
            gem::IntList{2187,23487,12520,13018,28115,25881,19082,6294,16260,22425,7479,12596,8073,20193,17548,65024},
            gem::IntList{12326,22333,16919,9088,21007,25862,7254,10769,10482,13598,7132,3418,15303,19038,13382,10254,11203} };
        ...
        bool optimzalPosition = false;       // the orders will be added at the end of the route
        gem::vrp::Request request;
        res = route.addOrders(&listener, ordersToAdd, optimzalPosition, request, reoptimize, { std::make_pair(gem::vrp::EVehicleType::VT_Car,distancesToTheOtherOrders) }, { std::make_pair(gem::vrp::EVehicleType::VT_Car,timesToTheOtherOrders) });
