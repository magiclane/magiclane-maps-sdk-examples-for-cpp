...
- Perform a stateless optimization using a single JSON input with orders in sequence pairs set (the orders from each pair will be visited consecutively in the specified order).  
...
1. Create a JSON object containing all the optimization data:
   ...
   - **Order sequence options** (sequence pairs that must be visited consecutively in the specified order)
